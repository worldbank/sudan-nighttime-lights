...
ntl_df <- readRDS(file.path(data_dir, "viirs-gee", "finaldata", "sudan_ntl_monthly.Rds"))

ntl_wide_df <- ntl_df %>%
  dplyr::mutate(period = case_when(
    date == ymd("2023-12-01") ~ "p1",
    date == ymd("2024-01-01") ~ "p1",
    date == ymd("2024-02-01") ~ "p1",
    
    date == ymd("2024-12-01") ~ "p2",
    date == ymd("2025-01-01") ~ "p2",
    date == ymd("2025-02-01") ~ "p2"
  )) %>%
  filter(!is.na(period)) %>%

...
#### HDX Boundaries
if(F){
  hdx2_sf <- read_sf(file.path(data_dir, "hdx-boundaries", "sdn_adm_cbs_nic_ssa_20200831.gdb"),
                     "sdn_admbnda_adm2_cbs_nic_ssa_20200831")
  saveRDS(hdx2_sf, file.path(data_dir, "hdx-boundaries", "adm2_hdx.Rds"))
}
...
ntl_months <- seq.Date(ymd("2012-01-01"), to = ymd("2025-06-01"), by = "month") %>%
  as.character()
  ...
  for(time_type in c("annual", "monthly")){
    ...
    for(time_i in time_vec){
      ...
      if(!file.exists(OUT_FILE)){
        ...
        if(unit == "hdx_adm2"){
          roi_sf <- readRDS(file.path(data_dir, "hdx-boundaries", "adm2_hdx.Rds"))
        }
...
## Aggregate Nighttime Lights - colorado school of mines
```{r}
#| warning: false
#| message: false
...
for(unit in c("adm0", 
  ...
  for(time_type in c("annual")){
    ...
    for(time_i in time_vec){
      ...
      #### Only process if file doesn't exist
      dir.create(file.path(data_dir, "viirs-gee", "finaldata", "aggregated_individual_files", unit))
      dir.create(file.path(data_dir, "viirs-gee", "finaldata", "aggregated_individual_files", unit, time_type))
      
      OUT_FILE <- file.path(data_dir, "viirs-gee", "finaldata", "aggregated_individual_files", unit, time_type, 
                            paste0("ntl_", time_i, ".Rds"))
      ...
      if(!file.exists(OUT_FILE)){
        ...
        ntl_r <- rast(file.path(data_dir, "viirs-gee", "rawdata", time_type, 
                                paste0("SDN_viirs_corrected_mean_",time_i,".tif")))
...
for(unit in c("adm0", 
  ...
  for(time_type in c("annual", "monthly")){
    ...
    ntl_df <- file.path(data_dir, "viirs-gee", "finaldata", "aggregated_individual_files", unit, time_type) %>%
      list.files(full.names = T,
                 pattern = ".Rds") %>%
      map_df(readRDS)
    
    saveRDS(ntl_df,   file.path(data_dir, "viirs-gee", "finaldata", "aggregated", paste0("ntl_", unit, "_", time_type, ".Rds")))
    write_csv(ntl_df, file.path(data_dir, "viirs-gee", "finaldata", "aggregated", paste0("ntl_", unit, "_", time_type, ".csv")))
    write_dta(ntl_df, file.path(data_dir, "viirs-gee", "finaldata", "aggregated", paste0("ntl_", unit, "_", time_type, ".dta")))
...
ntl_df %>%
  ...
  ggplot() +
  geom_vline(xintercept = ymd("2024-04-01"), color = "red") +
  geom_col(aes(x = date, y = ntl_sum),
           fill = wbg_color_light) +
  facet_wrap(~COUNTRY) +
  labs(x = NULL,
...
ntl_wide_df <- ntl_wide_df %>%
  dplyr::mutate(pc21 = (yr2021 - base_year)/base_year * 100,
...
Percent change in NTL: Jan - March 2024, vs May - Dec 2024
...
Percent change in NTL: 2022, vs 2024

```{r}

## Load
adm_sf <- load_gadm(2)
ntl_df <- readRDS(file.path(data_dir, "viirs-gee", "finaldata", "aggregated", "ntl_adm2_annual.Rds"))

## Prep data
ntl_wide_df <- ntl_df %>%
  dplyr::mutate(period = case_when(
                  date %in% 2022 ~ "ntl_before",
                  date %in% 2024 ~ "ntl_after"
                )) %>%
...
## Load
adm_sf <- load_gadm(3)
ntl_df <- readRDS(file.path(data_dir, "viirs-gee", "finaldata", "aggregated", "ntl_adm3_annual.Rds"))
...
r_before <- file.path(data_dir, "viirs-gee", "rawdata", "annual", "SDN_viirs_corrected_mean_2022.tif") %>%
  rast()

r_after <- file.path(data_dir, "viirs-gee", "rawdata", "annual", "SDN_viirs_corrected_mean_2024.tif") %>%
  rast()
...
## Table of % Change: Before/After Conflict: 2022 vs 2024

The below tables show the percent change in lights from 2022 to 2024
...
make_table <- function(roi_sf, unit_name){
  ...
  roi_sf %>%
    dplyr::select(Name, ntl_before, ntl_after, pc) %>%
    dplyr::mutate(across(c(ntl_before, ntl_after, pc), ~ round(.x, 2))) %>%
    dplyr::rename("NTL: 2022" = ntl_before,
                  "NTL: 2024" = ntl_after,
                  "% Change"= pc) %>%
    DT::datatable() 
}
